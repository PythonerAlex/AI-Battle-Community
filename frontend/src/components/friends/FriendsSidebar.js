...
import { Tabs, Input, Button, Typography ,message} from 'antd';
...
function FriendsSidebar() {
  ...
  // 2. 显示 Modal 的入口
  const handleInviteClick = (username) => {
    setInviteTarget(username);
  ...
  const handleRoomSelect = (roomId) => {
    ...
    fetch(`${API_BASE_URL}/api/friends/invite/`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
      },
      body: JSON.stringify({ to: inviteTarget, room_id: roomId })
    }).then(async (res) => {
      const data = await res.json();
      if (res.ok) {
        message.success(`Invitation sent to ${inviteTarget}`);
      } else {
        message.error(data.error || 'Invite failed');
      }
    }).catch(() => {
        ...
        <TabPane tab="Online" key="3">
          <div style={{ maxHeight: 260, overflowY: 'auto' }}>
            <OnlineUsersTab
              ...
              onInvite={handleInviteClick} // ✅ 传入相同函数
            />
          </div>
        </TabPane>
      </Tabs>
